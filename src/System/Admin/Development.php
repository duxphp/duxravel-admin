...
class Development extends Common
{
    ...
    {
        $node = new Node();
        $data = $node->{'n-layout'}(function ($node) {
            $node->div(function ($node) {
                $node->div(function ($node) {
                    $node->div(function ($node) {
                        $node->div('PV')->class('flex-none rounded-l bg-blue-600 text-white text-xl flex items-center justify-center w-14 h-16');
                        $node->div(function ($node) {
                            $node->div('{{data.apiNumDay}}')->class('text-xl');
                            $node->div('今日访问量')->class('text-gray-500 text-xs');
                        })->class('px-4 flex-grow');
                        $node->div(function ($node) {
                            switch ($this->data['apiNumTrend']) {
                                case 2:
                                    $class = 'text-red-600';
                                    break;
                                case 1:
                            ...
                            $node->div(function ($node) {
                                $node->div('{{data.apiNumRate}}%');
                                switch ($this->data['apiNumTrend']) {
                                    case 2:
                                        $node->{'rich-text'}()->nodes('<svg xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4 " fill="none"
                                             viewBox="0 0 24 24"
                                             stroke="currentColor">
                            ...
                            })->class("text-sm flex items-center justify-end $class");
                            $node->div('7天占比')->class('text-gray-500 text-xs');
                        })->class('flex-none px-4 text-right');
                    })->class('flex shadow bg-white rounded items-center');

                    $node->div(function ($node) {
                        $node->div('RT')->class('flex-none rounded-l bg-red-600 text-white text-xl flex items-center justify-center w-14 h-16');
                        $node->div(function ($node) {
                            $node->div('{{data.apiTimeDay}}s')->class('text-xl');
                            $node->div('响应速度')->class('text-gray-500 text-xs');
                        })->class('px-4 flex-grow');
                        $node->div(function ($node) {
                            switch ($this->data['apiTimeTrend']) {
                                case 2:
                                    $class = 'text-red-600';
                                    break;
                                case 1:
                    ...
                    $node->div(function ($node) {
                        $node->div('FN')->class('flex-none rounded-l bg-yellow-600 text-white text-xl flex items-center justify-center w-14 h-16');
                        $node->div(function ($node) {
                            $node->div('{{data.fileNumDay || 0}}')->class('text-xl');
                            $node->div('文件数量')->class('text-gray-500 text-xs');
                        })->class('px-4 flex-grow');
                        $node->div(function ($node) {
                            switch ($this->data['fileNumTrend']) {
                                case 2:
                                    $class = 'text-red-600';
                                    break;
                                case 1:
                    ...
                    $node->div(function ($node) {
                        $node->div('LN')->class('flex-none rounded-l bg-green-600 text-white text-xl flex items-center justify-center w-14 h-16');
                        $node->div(function ($node) {
                            $node->div('{{data.logNumDay || 0}}')->class('text-xl');
                            $node->div('操作日志')->class('text-gray-500 text-xs');
                        })->class('px-4 flex-grow');
                        $node->div(function ($node) {
                            switch ($this->data['logNumTrend']) {
                                case 2:
                                    $class = 'text-red-600';
                                    break;
                                case 1:
                ...
                })->class('grid grid-cols-1 xl:grid-cols-4 lg:grid-cols-2 gap-5 whitespace-nowrap');

                $node->div(function ($node) {
                    $node->div($this->node['apiNumChart'])->class('shadow bg-white rounded p-5');
                    $node->div($this->node['apiTimeChart'])->class('shadow bg-white rounded p-5');
                    $node->div($this->node['fileNumChart'])->class('shadow bg-white rounded p-5');
                    $node->div($this->node['logNumChart'])->class('shadow bg-white rounded p-5');
                })->class('grid grid-cols-1 xl:grid-cols-4 lg:grid-cols-2 gap-5 whitespace-nowrap mt-5');

                $node->div(function ($node) {
                    $node->div(function ($node) {
                        $node->div(function ($node) {
                            $node->span('访问排名')->class('text-base flex-grow');
                            $node->span(function ($node) {
                                $node->{'n-dropdown'}(function ($node) {
                                    $node->{'n-button'}('最近 {{config.filter.views.day}} 天');
                                })->{'vOn:select'}("handleSelect(\$event, 'views')")->{'vBind:options'}("optionDay('views')");
                            })->class('flex-none select-none');
                        })->class('flex items-center relative');
                        $node->{'app-table'}()
                            ->url(route("admin.system.visitorApi.ajax", ['limit' => 10, '_time' => random_int(1, 10000)]))
                            ->{'vBind:columns'}('config.columnsTotal')
                            ->{'vBind:filter'}('config.filter.views')
                            ->{'vBind:n-params'}('config.table')
                            ->class('mt-4');
                    })->class('bg-white rounded shadow p-5');

                    $node->div(function ($node) {
                        $node->div(function ($node) {
                            $node->span('响应排名')->class('text-base flex-grow');
                            $node->span(function ($node) {
                                $node->{'n-dropdown'}(function ($node) {
                                    $node->{'n-button'}('最近 {{config.filter.response.day}} 天');
                                })->{'vOn:select'}("handleSelect(\$event, 'response')")->{'vBind:options'}("optionDay('response')");
                            })->class('flex-none select-none');
                        })->class('flex items-center relative');
                        $node->{'app-table'}()
                            ->url(route("admin.system.visitorApi.ajax", ['limit' => 10, '_time' => random_int(1, 10000)]))
                            ->{'vBind:columns'}('config.columnsResponse')
                            ->{'vBind:filter'}('config.filter.response')
                            ->{'vBind:n-params'}('config.table')
                            ->class('mt-4');
                    })->class('bg-white rounded shadow p-5');

                })->class('grid grid-cols-1 xl:grid-cols-2 gap-5 mt-5');

                $node->div(function ($node) {

                    $node->div(function ($node) {
                        $node->div(function ($node) {
                            $node->span('操作日志')->class('text-base flex-grow');
                            $node->span(function ($node) {
                                $node->{'n-dropdown'}(function ($node) {
                                    $node->{'n-button'}('最近 {{config.filter.log.day}} 天');
                                })->{'vOn:select'}("handleSelect(\$event, 'log')")->{'vBind:options'}("optionDay('log')");
                            })->class('flex-none select-none');
                        })->class('flex items-center relative');
                        $node->{'app-table'}()
                            ->url(route("admin.system.operate.ajax", ['limit' => 9, '_time' => random_int(1, 10000)]))
                            ->{'vBind:columns'}('config.columnsLog')
                            ->{'vBind:filter'}('config.filter.log')
                            ->{'vBind:n-params'}('config.table')
                            ->class('mt-4');
                        })->class('bg-white rounded shadow p-5');
                    ...
                    $node->div(function ($node) {
                        $node->div(function ($node) {
                            $node->div('环境信息')->class('text-base text-gray-600');
                            $node->div('系统运行各参数与环境数据')->class('mt-1 text-sm text-gray-500');
                        })->class('px-4 py-5 sm:px-6');

                        $node->div(function ($node) {
                            $node->dl(function ($node) {

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1607399448502" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                          ...
                                          <path
                                            ...
                                            p-id="2811"></path>
                                        </svg>')->class('bg-red-600 p-2 rounded text-white');
                                        $node->div('Laravel 版本')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(app()->version())->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1607399651954" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                        ...
                                        </svg>')->class('bg-gray-700 p-2 rounded text-white');
                                        $node->div('操作系统')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(PHP_OS_FAMILY . ' Version ' . php_uname('r'))->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1607399504569" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                        ...
                                        </svg>')->class('bg-blue-600 p-2 rounded text-white');
                                        $node->div('PHP版本')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(php_sapi_name() . ' ' . PHP_VERSION)->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1607399567811" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                        ...
                                        </svg>')->class('bg-yellow-600 p-2 rounded text-white');
                                        $node->div('Mysql版本')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(DB::select('SHOW VARIABLES LIKE "version"')[0]->Value)->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1623898130568" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                        ...
                                        $node->div('Redis版本')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(\Illuminate\Support\Facades\Redis::info()['redis_version'])->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1623898416456" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                          ...
                                          <path
                                            ...
                                            p-id="3438"></path>
                                        </svg>')->class('bg-green-600 p-2 rounded text-white');
                                        $node->div('服务器IP')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd($_SERVER['SERVER_ADDR'])->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1607661922912" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                        ...
                                        </svg>')->class('bg-purple-600 p-2 rounded text-white');
                                        $node->div('Web服务端')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd($_SERVER['SERVER_SOFTWARE'])->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1610011408252" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                        ...
                                        $node->div('上传限制')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(get_cfg_var ("upload_max_filesize") ? get_cfg_var ("upload_max_filesize") : "不允许")->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1610011470374" class="w-5 h-5 fill-current" viewBox="0 0 1029 1024"
                                             version="1.1"
                                        ...
                                        $node->div('脚本超时')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(get_cfg_var("max_execution_time") . 's')->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');

                                $node->div(function ($node) {
                                    $node->dt(function ($node) {
                                        $node->{'rich-text'}()->nodes('<svg t="1610011525042" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024"
                                             version="1.1"
                                        ...
                                        $node->div('剩余空间')->class('ml-2');
                                    })->class('text-sm text-gray-600 flex items-center flex-grow lg:flex-none');
                                    $node->dd(round(disk_free_space(base_path()) /1024/1024/1024, 2) . "GB")->class('lg:mt-1 text-sm text-gray-600 sm:mt-0 sm:col-span-2');
                                })->class('px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center');
        ...
        return [
            ...
            <<<JS
                ...
                const handleSelect = function(key, name) {
                  config.value.filter[name].day = key
                }
