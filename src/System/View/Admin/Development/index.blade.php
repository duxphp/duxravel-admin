<div class="p-5">
    <div class="grid grid-cols-1 xl:grid-cols-4 lg:grid-cols-2 gap-5 whitespace-nowrap">
        <div class=" flex shadow bg-white rounded items-center">
            <div class="flex-none rounded-l bg-blue-900 text-white text-xl flex items-center justify-center w-14 h-16">
                PV
            </div>
            ...
            <div class="flex-none px-4 text-right">
                <div
                        class="text-sm flex items-center justify-end {{$apiNumTrend === 1 ? 'text-red-900' : ''}} {{$apiNumTrend === 2 ? 'text-green-900' : ''}} {{$apiNumTrend === 0 ? 'text-blue-900' : ''}} ">
                    <div>{{$apiNumRate}}%</div>
                    @if($apiNumTrend === 2)
        ...
        <div class=" flex shadow bg-white rounded items-center">
            <div class="flex-none rounded-l bg-red-900 text-white text-xl flex items-center justify-center w-14 h-16">
                RT
            </div>
            ...
            <div class="flex-none px-4 text-right">
                <div
                        class="text-sm flex items-center justify-end {{$apiTimeTrend === 2 ? 'text-red-900' : ''}} {{$apiTimeTrend === 1 ? 'text-green-900' : ''}} {{$apiTimeTrend === 0 ? 'text-blue-900' : ''}} ">
                    <div>{{$apiTimeRate}}%</div>
                    @if($apiTimeTrend === 1)
                        <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4 " fill="none" viewBox="0 0 24 24"
                             stroke="currentColor">
        ...
        <div class=" flex shadow bg-white rounded items-center">
            <div class="flex-none rounded-l bg-yellow-900 text-white text-xl flex items-center justify-center w-14 h-16">
                FN
            </div>
            <div class="flex-grow px-4">
            ...
            <div class="flex-none px-4 text-right">
                <div
                        class="text-sm flex items-center justify-end {{$fileNumTrend === 1 ? 'text-red-900' : ''}} {{$fileNumTrend === 2 ? 'text-green-900' : ''}} {{$fileNumTrend === 0 ? 'text-blue-900' : ''}} ">
                    <div>{{$fileNumRate}}%</div>
                    @if($fileNumTrend === 2)
                        <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4 " fill="none" viewBox="0 0 24 24"
                             stroke="currentColor">
        ...
        <div class=" flex shadow bg-white rounded items-center">
            <div class="flex-none rounded-l bg-green-900 text-white text-xl flex items-center justify-center w-14 h-16">
                LN
            </div>
            <div class="flex-grow px-4">
            ...
            <div class="flex-none px-4 text-right">
                <div
                        class="text-sm flex items-center justify-end {{$logNumTrend === 1 ? 'text-red-900' : ''}} {{$logNumTrend === 2 ? 'text-green-900' : ''}} {{$logNumTrend === 0 ? 'text-blue-900' : ''}} ">
                    <div>{{$logNumRate}}%</div>
                    @if($fileNumTrend === 2)
                        <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4 " fill="none" viewBox="0 0 24 24"
                             stroke="currentColor">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 lg:grid-cols-2 gap-5 whitespace-nowrap mt-5">
        <div class=" shadow bg-white rounded p-5">
            <div id="api-num-chart"></div>
        </div>
        <div class=" shadow bg-white rounded p-5">
            <div id="api-time-chart"></div>
        </div>
        <div class=" shadow bg-white rounded p-5">
            <div id="file-num-chart"></div>
        </div>
        <div class=" shadow bg-white rounded p-5">
            <div id="log-num-chart"></div>
        </div>
    </div>
                ...
                <div class="flex-none select-none" x-data="system.loadArea({data: [7, 30, 90], active: 7, target: '#api-total', url: '{{ route("admin.system.visitorApi.loadTotal") }}'})" x-init="loadData()">
                    ...
                    <ul x-spread="menu"
                        ...
                        <template x-for="item in data">
                            <li>
                                <a href="javascript:;" @click="select(item)" class="block p-2 hover:bg-gray-200">最近<span x-text="item"></span>天</a>
                            </li>
                        </template>
                    </ul>
                </div>
            </div>
            <div id="api-total"  class="overflow-x-scroll mt-4"></div>
        </div>
        <div class="bg-white rounded shadow p-5">
            <div class="flex items-center relative">
            ...
            <div id="api-delay" class="overflow-x-scroll mt-4"></div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-5 mt-5">
        <div class="bg-white rounded shadow p-5">
            ...
            <div id="api-log" class="overflow-x-scroll mt-4"></div>
        </div>
                ...
                <dl>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                        <dt class="text-sm text-gray-600 flex items-center flex-grow lg:flex-none">
                            <div class="bg-red-900 p-2 rounded text-white">
                        ...
                        <dd class="lg:mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            {{ app()->version() }}
                        </dd>
                    </div>
                    ...
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                        <dt class="text-sm text-gray-600 flex items-center flex-grow lg:flex-none">
                            <div class="bg-blue-900 p-2 rounded text-white">
                    ...
                    <div class="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                        <dt class="text-sm text-gray-600 flex items-center flex-grow lg:flex-none">
                            <div class="bg-yellow-900 p-2 rounded text-white">
                    ...
                    <div class="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                        <dt class="text-sm text-gray-600 flex items-center flex-grow lg:flex-none">
                            <div class="bg-green-900 p-2 rounded text-white">
                                <svg t="1623898416456" class="w-5 h-5 fill-current" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3437" width="200" height="200"><path d="M843.861333 938.666667H180.138667C127.573333 938.666667 85.333333 896 85.333333 843.861333V180.138667C85.333333 128 127.530667 85.333333 180.138667 85.333333h663.722666C896 85.333333 938.666667 128 938.666667 180.138667v663.722666C938.666667 896 896 938.666667 843.861333 938.666667zM393.472 322.389333H298.666667v379.221334h94.805333V322.389333z m355.584 47.36c0-26.154667-21.248-47.36-47.445333-47.36h-213.333334v379.221334h94.848v-142.208h118.485334A47.36 47.36 0 0 0 749.056 512V369.792z m-165.973333 23.722667h94.848v94.805333h-94.805334V393.472z" p-id="3438"></path></svg>
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                        <dt class="text-sm text-gray-600 flex items-center flex-grow lg:flex-none">
                            <div class="bg-purple-900 p-2 rounded text-white">
